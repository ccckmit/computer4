...
while offset < sh_size:
    b0 = data[offset]
    b1 = data[offset+1] if offset+1 < sh_size else 0
    if (b0 & 0x3) != 0x3:
        inst = b0 | (b1 << 8)
        op = inst & 0x3
        funct3 = (inst >> 13) & 0x7
        ...
        if op == 0 and funct3 == 0:
            ...
            if imm == 0: name = 'C.NOP (reserved)'
            else: name = 'C.ADDI4SPN rd=x{} imm={}'.format(rdp, imm)
        elif op == 0 and funct3 == 2:
            imm = (((inst >> 5) & 0x38) | ((inst >> 1) & 4) | ((inst >> 7) & 4))
            name = 'C.LW rd=x{} offset={}(x{})'.format(rdp, imm, rs1p)
        elif op == 0 and funct3 == 6:
            imm = (((inst >> 5) & 0x38) | ((inst >> 1) & 4) | ((inst >> 7) & 4))
            name = 'C.SW rs2=x{} offset={}(x{})'.format(rs2p, imm, rs1p)
        elif op == 1 and funct3 == 0:
            imm = sext(((inst >> 7) & 0x20) | ((inst >> 2) & 0x1f), 6)
            name = 'C.ADDI rd=x{} imm={}'.format(rd, imm)
        elif op == 1 and funct3 == 1 and rd == 0:
            name = 'C.NOP'
        elif op == 1 and funct3 == 2:
            imm = sext(((inst >> 7) & 0x20) | ((inst >> 2) & 0x1f), 6)
            name = 'C.LI rd=x{} imm={}'.format(rd, imm)
        elif op == 1 and funct3 == 3:
            if rd == 2:
                imm = sext(((inst >> 6) & 0x20) | ((inst >> 3) & 0x10) | ((inst >> 5) & 8) | ((inst << 1) & 0x100) | ((inst >> 2) & 0x40) | ((inst >> 1) & 0x180), 9)
                name = 'C.ADDI16SP imm={}'.format(imm)
            else:
                imm = ((inst >> 12) & 1) << 5 | ((inst >> 2) & 0x1f)
                name = 'C.LUI rd=x{} imm=0x{:x}'.format(rd, imm)
        elif op == 1 and funct3 == 4:
            fa = (inst >> 12) & 3
            sa = (inst >> 5) & 3
            if fa==0 and sa==0: name = 'C.SRLI x{}'.format(rs1p)
            elif fa==0 and sa==1: name = 'C.SRAI x{}'.format(rs1p)
            elif fa==1 and sa==0: name = 'C.ANDI x{}'.format(rs1p)
            elif fa==2 and sa==0: name = 'C.SUB x{}'.format(rs1p)
            elif fa==2 and sa==1: name = 'C.XOR x{}'.format(rs1p)
            elif fa==2 and sa==2: name = 'C.OR x{}'.format(rs1p)
            elif fa==2 and sa==3: name = 'C.AND x{}'.format(rs1p)
            elif fa==3 and sa==0: name = 'C.SUBW x{}'.format(rs1p)
            elif fa==3 and sa==1: name = 'C.ADDW x{}'.format(rs1p)
            else: name = 'C.RESERVED'
        elif op == 1 and funct3 == 5: name = 'C.J'
        elif op == 1 and funct3 == 6: name = 'C.BEQZ'
        elif op == 1 and funct3 == 7: name = 'C.BNEZ'
        elif op == 2 and funct3 == 0:
            sh = ((inst >> 7) & 0x3c) | ((inst >> 2) & 3)
            name = 'C.SLLI rd=x{} sh={}'.format(rd, sh)
        elif op == 2 and funct3 == 2: name = 'C.LWSP rd=x{}'.format(rd)
        elif op == 2 and funct3 == 4:
            r1 = (inst >> 7) & 0x1f
            r2 = (inst >> 2) & 0x1f
            if r2 == 0:
                name = 'C.JR x{}'.format(r1) if r1 != 0 else 'C.EBREAK'
            else: name = 'C.MV x{}=x{}'.format(r1, r2)
        elif op == 2 and funct3 == 6: name = 'C.SWSP rs2=x{}'.format(rs2)
        elif op == 2 and funct3 == 7: name = 'C.SDSP rs2=x{}'.format(rs2)
        elif op == 0 and funct3 == 3: name = 'C.LD (RV64)'
        elif op == 0 and funct3 == 7: name = 'C.SD (RV64)'
        elif op == 2 and funct3 == 3: name = 'C.LDSP (RV64)'
        else: name = 'UNKNOWN op={} funct3={}'.format(op, funct3)
        print('  {:#06x}: {:#06x}  {}'.format(offset, inst, name))
        offset += 2
    else:
        inst = b0 | (b1 << 8) | (data[offset+2] << 16) | (data[offset+3] << 24)
        opcode = inst & 0x7f
        rd_v = (inst >> 7) & 0x1f
        ...
        print('  {:#06x}: {:#010x}  {}'.format(offset, inst, name))
        offset += 4
    if offset >= sh_size:
        break
