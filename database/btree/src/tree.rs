...
impl<S: Storage> BPlusTree<S> {
    ...
    fn delete_recursive(&mut self, idx: usize, key: &Key) -> bool {
        let node = self.storage.read_node(idx);
        if node.is_leaf() {
            return self.delete_from_leaf(idx, node, key);
        }
        let pos = node.keys.partition_point(|k| k <= key);
        let child_idx = node.children[pos];
        if !self.delete_recursive(child_idx, key) {
            return false;
        }
    ...
    fn rebalance(&mut self, parent: usize, pos: usize) {
        ...
        if pos + 1 < n_children {
            self.merge(parent, pos);
        } else {
            self.merge(parent, pos - 1);
        }
    }
    ...
    fn borrow_from_left(&mut self, parent: usize, pos: usize) {
        ...
        if child.is_leaf() {
            ...
            child.keys.insert(0, lk);
            child.records.insert(0, lr);
        } else {
    ...
    fn merge(&mut self, parent: usize, left_pos: usize) {
        ...
        pn.keys.remove(left_pos);
        pn.children.remove(left_pos + 1);
        self.storage.write_node(lid, &left);
        self.storage.write_node(parent, &pn);
    }
}
...
mod tests {
    ...
    fn new_tree() -> BPlusTree<MemoryStorage> {
        BPlusTree::new(4, MemoryStorage::new())
    }
    ...
    fn test_insert_many_triggers_split() {
        let mut tree = new_tree();
        for i in 0..20i64 {
            tree.insert(int_key(i), val("x"));
        }
        assert_eq!(tree.len(), 20);
        for i in 0..20i64 {
            assert!(
                tree.search(&int_key(i)).is_some(),
                "key {} missing",
                i
            );
        }
    ...
    fn test_text_key() {
        ...
        assert_eq!(
            tree.search(&Key::Text("banana".into())),
            Some(b"fruit".to_vec())
        );
    }
    ...
    fn test_disk_storage_persist() {
        ...
        let path = "/tmp/btree_persist_test.sql4db";
        ...
        let root_page;
        {
            ...
            for i in 1..=5i64 {
                tree.insert(int_key(i), val("persisted"));
            }
