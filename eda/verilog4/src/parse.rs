...
impl Parser {
    ...
    fn parse_module(&mut self) -> Module {
        ...
        if self.at(Token::Semicolon) {
            ...
            self.pos += 1;
            let (port_decls, items) = self.parse_port_decls_and_body();
    ...
    fn parse_port_decls_and_body(&mut self) -> (Vec<(String, PortDir, Option<Range>)>, Vec<ModuleItem>) {
            ...
            match self.toks.get(self.pos) {
                ...
                Some(Token::Output) => {
                    ...
                    for n in self.parse_id_list() {
                        ...
                        if is_reg { items.push(ModuleItem::Reg(VarDecl { name: n, width: w.clone() })); }
                    }
                }
        ...
        loop {
            ...
            match self.toks.get(self.pos) {
                Some(Token::Wire) => { self.pos += 1; let w = self.parse_range_opt(); for n in self.parse_id_list() { items.push(ModuleItem::Wire(VarDecl { name: n, width: w.clone() })); } }
                Some(Token::Reg) => { self.pos += 1; let w = self.parse_range_opt(); for n in self.parse_id_list() { items.push(ModuleItem::Reg(VarDecl { name: n, width: w.clone() })); } }
